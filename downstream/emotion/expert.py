...
class DownstreamExpert(nn.Module):
    ...
    def __init__(self, upstream_dim, downstream_expert, expdir, **kwargs):
        ...
        # 构建esd数据集
        esd_path = os.path.join(
            '/sdc/home/mazening/model/interspeech2023/downstream/emotion/meta_data/ESD', 'esd_meta_data.json')
        print(f'[Expert] - ESD path: {esd_path}')
        self.esd_dataset = IEMOCAPDataset('/sdc/home/mazening/dataset/ESDcorpus', esd_path, self.datarc['pre_load'])
    ...
    # 添加ESD数据集进行测试
    def _get_esd_dataloader(self, dataset):
        # 实现获取 "esd" 数据集的 DataLoader 的逻辑
        # 例如，可以使用与训练数据集获取 DataLoader 相似的逻辑
        sampler = DistributedSampler(dataset) if is_initialized() else None
        return DataLoader(
            dataset, batch_size=self.datarc['esd_batch_size'],
            shuffle=(sampler is None), sampler=sampler,
            num_workers=self.datarc['num_workers'],
            collate_fn=collate_fn
        )

    def get_esd_dataloader(self):
        # 获取 "esd" 数据集的 DataLoader
        return self._get_esd_dataloader(self.esd_dataset)
