...
class Runner():
    ...
    def __init__(self, args, config):
        self.args = args
        self.config = config
        self.logger = SummaryWriter('/sdc/home/mazening/model/interspeech2023/result/pretrain/npc_score_m=7')
    ...
    def train(self):
        ...
        score_loss = 0
        # awe_loss = 0
        # writer = SummaryWriter("original_libri")
        tensorboard_step = 0
            ...
            for data in tqdm(dataloader, dynamic_ncols=True, desc='train'):
                ...
                try:
                    ...
                    if gradient_accumulate_steps > 1:
                        ...
                        # loss_awe = loss_awe / gradient_accumulate_steps
                        loss_score = loss_score / gradient_accumulate_steps
                    if self.args.multi_gpu:
                        loss_mam = loss_mam.sum()
                        # loss_awe = loss_awe.sum()
                        loss_score = loss_score.sum()
                    if amp:
                        scaler.scale(loss_mam).backward()
                        # scaler.scale(loss_awe).backward()
                        scaler.scale(loss_score).backward()
                    else:
                        # loss_mam = loss_mam.detach_().requires_grad_(True)
                        # loss_awe = loss_awe.detach_().requires_grad_(True)
                        # loss_score = loss_score.detach_().requires_grad_(True)
                        loss_mam.backward()
                        # loss_awe.backward()
                        loss_score.backward()
                ...
                # record loss
                # loss_all = loss_mam.item() + loss_awe.item() + loss_score.item()

                # 只包含情感分数信息
                # loss_all = loss_mam.item() + loss_score.item()

                # 只包含声学词向量信息
                # loss_all = loss_mam.item() + loss_awe.item()
                loss_all = loss_mam.item() + loss_score.item()
                add_loss += loss_all
                mam_loss += loss_mam.item()
                # awe_loss += loss_awe.item()
                ...
                if global_step % self.config['runner']['log_step'] == 0 or pbar.n == pbar.total - 1:
                    # log loss
                    self.logger.add_scalar(f'{prefix}all_loss', loss_all, global_step=global_step)
                    self.logger.add_scalar(f'{prefix}mam_loss', mam_loss, global_step=global_step)
                    # self.logger.add_scalar(f'{prefix}awe_loss', awe_loss, global_step=global_step)
                    self.logger.add_scalar(f'{prefix}score_loss', score_loss, global_step=global_step)
                ...
                if global_step % self.config['runner']['save_step'] == 0 or pbar.n == pbar.total - 1:
                    ...
                    save_path = os.path.join(
                        '/sdc/home/mazening/model/interspeech2023/result/pretrain/npc_score_m=7', name)
